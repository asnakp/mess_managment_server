...
module.exports = {
    ...
    funGetAllOrderDetails: GetAllOrderDetails=(obj,db)=> {
        return new Promise((resolve, reject) => {
            try{
                ...
                db.collection(config.ORDER_COLLECTION).find(query).count()
                    .then((totalPageCount) => {
                        if(totalPageCount){
                            if(!intPageLimit)
                                intPageLimit =parseInt(totalPageCount);
                            db.collection(config.ORDER_COLLECTION).aggregate([{$match:query},
                                {$sort:{date:-1}},
        ...
        funGetByOrderIdDetails: GetByOrderDetails=(obj,db)=> {
            console.log("222222222222222 ---",obj)
            return new Promise((resolve, reject) => {
                try{
                    var arrayAllObjData =[];
                    var query= { _id: ObjectID(obj.intOrderId) , strStatus: "N" }
